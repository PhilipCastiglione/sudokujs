var rules = {
  ...
  updateXPossibilitiesOfYCombinations: function() {
    if (!this.numberCombinations) { // using state to save lots of cpu cycles
  ...
  hulkSmash: function() {
    if (game.hulkActive) {
      console.log('hulk');
      console.log('hulkIndex: ' + game.hulkIndex);
      console.log('hulkNumber: ' + game.hulkNumber);
      if (game.isWon()) {
        alert('hulkwin');
      } else {
        if (board.cells[game.hulkIndex].indexOf(game.hulkNumber) === 
            board.cells[game.hulkIndex].length - 1) {
          ...
          var localIndex = board.cells[game.hulkIndex].indexOf(game.hulkNumber);
          game.hulkNumber = board.cells[game.hulkIndex][localIndex + 1];
        }
      ...
      console.log('hulkNumber: ' + game.hulkNumber);
      game.solve();
      console.log('first hulk resolved');
    }
  },
  nextHulkIndex: function() {
    console.log('beginning idx: ' + game.hulkIndex);
    if (game.hulkIndex === undefined) {
      game.hulkIndex = 0;
    } else {
      game.hulkIndex++;
    }
    ...
    console.log('ending idx: ' + game.hulkIndex);
  }
};
